...
# ============================================================
# Pure-MPI timing script — Brio-Wu & Orszag-Tang, HLLD, n=2 (weak scaling)
# Mirrors scripts/slurm_cpu_timing.sh; the only real difference is that
# main_cpu is launched directly while main_mpi is launched via mpirun
# with one rank per core (pure MPI, no OpenMP threading inside a rank —
# see Makefile: the "mpi" target deliberately omits -fopenmp).
# ============================================================
...
# Detect how many physical cores are available on this node, and use one
# MPI rank per core by default (override with RANKS=... sbatch ...).
detect_cpus_on_node() {
...
RANKS="${RANKS:-${CPUS_ON_NODE}}"

if [ "$RANKS" -gt "$CPUS_ON_NODE" ]; then
    echo "[ERROR] RANKS=${RANKS} > CPUS_ON_NODE=${CPUS_ON_NODE}"; exit 1
fi
...
# Summary CSV
# ------------------------------------------------------------------
SUMMARY="validation/mpi_timing_${SLURM_JOB_ID}.csv"
echo "case,solver,n,ranks,nx,ny,total_cells,steps,elapsed_s,steps_per_s,Mcell_updates_s,real_s,user_s,sys_s,max_rss_kb,git_branch,git_commit" \
    > "$SUMMARY"
...
run_case() {
    ...
    local out_dir="outputs/${SLURM_JOB_ID}/${case_name}_${SOLVER}_n${N_SCALE}_r${RANKS}"
    mkdir -p "$out_dir"

    echo "===== RUN: ${case_name} | solver=${SOLVER} | n=${N_SCALE} | ranks=${RANKS} ====="
    ...
    /usr/bin/time -f "real_seconds=%e\nuser_seconds=%U\nsys_seconds=%S\nmax_rss_kb=%M" \
        -o "$temp_time" \
        mpirun -np "$RANKS" ./bin/main_mpi "$case_name" --n "$N_SCALE" --solver "$SOLVER" --out "$out_dir" \
        2>&1 | tee "$temp_log"

    # Parse [TIMING] line from main_mpi output
    local n nx ny ranks_reported steps elapsed steps_per_s Mcell cells
    n=$(              parse_timing "$temp_log" "n")
    nx=$(             parse_timing "$temp_log" "nx")
    ny=$(             parse_timing "$temp_log" "ny")
    ranks_reported=$( parse_timing "$temp_log" "ranks")
    ...
    # Parse /usr/bin/time output (wraps the whole mpirun launch)
    ...
    echo "${case_name},${SOLVER},${n:-${N_SCALE}},${ranks_reported:-${RANKS}},${nx},${ny},${cells},${steps},${elapsed},${steps_per_s},${Mcell},${real},${user},${sys},${rss},${GIT_BRANCH},${GIT_COMMIT}" \
        >> "$SUMMARY"
...
tail -n +2 "$SUMMARY" | while IFS=',' read -r \
    case solver n ranks nx ny cells steps elapsed steps_per_s Mcell \
