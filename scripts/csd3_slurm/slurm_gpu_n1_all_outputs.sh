...
# CSD3 GPU full-output sweep for plotting:
#   cases   = Orszag-Tang, rotor, Brio-Wu
#   solvers = HLL, HLLC, HLLD, FORCE
#   n       = 1
#
# The GPU executable is built once and then used for all 12 runs. Each run
# writes its complete field snapshots to a separate directory.
#
# Submit from the MHD repository root:
#   mkdir -p logs
#   sbatch scripts/csd3_slurm/slurm_gpu_n1_all_outputs.sh
#
# Optional overrides:
...
#   sbatch scripts/csd3_slurm/slurm_gpu_n1_all_outputs.sh
#
# To reproduce a non-default GPU build, pass its compiler definitions:
...
if [ ! -f "${WORKDIR}/Makefile" ] || [ ! -f "${WORKDIR}/scripts/gpu/main_gpu.cu" ]; then
    ...
    echo "[ERROR] Submit from the repository root or set WORKDIR explicitly."
    exit 1
fi
...
for case_name in "${CASES[@]}"; do
    case "${case_name}" in
        ...
        *) echo "[ERROR] Unknown case '${case_name}'."; exit 2 ;;
    esac
done
...
OUT_ROOT="${WORKDIR}/outputs/csd3_gpu_n1_plots/${SLURM_JOB_ID}"
BUILD_ROOT="${WORKDIR}/build/csd3_gpu_n1_plots/${SLURM_JOB_ID}"
BIN_ROOT="${WORKDIR}/bin/csd3_gpu_n1_plots/${SLURM_JOB_ID}"
...
if [[ "${COMPUTE_CAP}" =~ ^[0-9]+\.[0-9]+$ ]]; then
    ...
    case "${GPU_NAME}" in
        ...
        *)
            ...
            echo "[ERROR] Set CUDA_ARCH_FLAG explicitly, for example --arch=sm_80."
            exit 1
            ;;
    esac
fi
...
for case_name in "${CASES[@]}"; do
    for solver in "${SOLVERS[@]}"; do
        ...
        relative_output="outputs/csd3_gpu_n1_plots/${SLURM_JOB_ID}/${label}"
