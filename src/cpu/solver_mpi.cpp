...
Grid2D make_local_mpi_grid(
    ...
    phys::gamma = cfg.gamma;  // must be set before any physics calls
...
double compute_dt_mpi(const Grid2D& grid, const MpiDecomp2D& mp, double cfl) {
    ...
    // Set GLM cleaning speed to max global signal speed (all ranks share same value)
    phys::ch_glm = max_speed_global;
...
void advance_second_order_mpi(
    ...
    // Mixed-GLM ψ damping (Dedner eq. 45): psi *= exp(-dt*ch/cr)
    {
